...
import Base from '../core/Base'
 ...
 import {getAProducts,getAllCategory,updateProduct} from './helper/adminapicall'
const UpdateProduct=({match})=>{
    ...
    const preLoad=(productId)=>{
        getAProducts(productId).then(data=>{
            console.log(data)
            if(data.err)
            {
    ...
    const successMessage=()=>{
        return(
            ...
            style={{display:craetedProduct ?"" :"none"}}>
                <h4>{craetedProduct} updated successfully</h4>
    ...
    const errorMessage=()=>{
     ...
     const onSubmit=event=>{
        ...
        updateProduct(match.params.productId,user._id,token,fromData)
        .then(data=>{
            if(data.err)
            {
